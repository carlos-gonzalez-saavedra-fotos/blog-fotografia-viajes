...
interface LightboxProps {
  ...
  ubicacion: string;
  /** Contenido adicional que se renderiza debajo del caption (ej: link a crónica) */
...
export const Lightbox: React.FC<LightboxProps> = ({
  ...
  // Gestión de body overflow
  useEffect(() => {
    document.body.style.overflow = isOpen ? 'hidden' : 'unset';
    return () => { document.body.style.overflow = 'unset'; };
  }, [isOpen]);

  // Reset portrait en cambio de foto
  useEffect(() => {
    setIsPortrait(false);
  }, [currentIndex]);
    ...
    <AnimatePresence>
      ...
      >
        ...
        {photos.length > 1 && (
          <>
            <button 
              className="hidden lg:block absolute left-8 top-1/2 -translate-y-1/2 text-white/20 hover:text-gold z-50 transition-all" 
              onClick={(e) => { e.stopPropagation(); onNavigate(-1); }}
            >
              <ChevronLeft size={64} strokeWidth={1} />
            </button>
            <button 
              className="hidden lg:block absolute right-8 top-1/2 -translate-y-1/2 text-white/20 hover:text-gold z-50 transition-all" 
              onClick={(e) => { e.stopPropagation(); onNavigate(1); }}
            >
        ...
        <div className="flex-grow flex items-center justify-center p-4">
          ...
          >
            ...
            <div className="mt-6 text-center w-full px-4">
              ...
              {/* Footer personalizable (ej: link a crónica) */}
