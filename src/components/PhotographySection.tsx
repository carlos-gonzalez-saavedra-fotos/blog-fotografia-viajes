...
export const PhotographySection = () => {
  ...
  React.useEffect(() => { setIsPortrait(false); }, [subIndex]);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="columns-1 md:columns-2 lg:columns-3 gap-8 space-y-8">
          {allGalleries.map((photo) => (
            <div key={photo.id} onClick={() => { setSelectedId(photo.id); setSubIndex(0); }} className="relative group cursor-zoom-in overflow-hidden">
        ...
        {selectedId && (
          <div className="fixed inset-0 z-[9999] bg-black/98 flex flex-col" onClick={() => setSelectedId(null)}>
            <div className="w-full px-6 py-4 md:px-10 flex justify-between items-center z-10">
              <div className="flex items-center gap-2"><Camera className="w-5 h-5 text-gold" /><span className="font-serif text-lg uppercase text-white hidden sm:inline">CGS</span></div>
              <button onClick={() => setSelectedId(null)} className="text-white hover:text-gold p-3 transition-all hover:rotate-90"><X size={24} /></button>
            </div>

            {currentGallery.length > 1 && (
              <>
                <button className="hidden lg:block absolute left-6 top-1/2 -translate-y-1/2 text-white/20 hover:text-gold z-[10000] p-4 transition-all" onClick={(e) => { e.stopPropagation(); navigate(-1); }}><ChevronLeft size={56} strokeWidth={1} /></button>
                <button className="hidden lg:block absolute right-6 top-1/2 -translate-y-1/2 text-white/20 hover:text-gold z-[10000] p-4 transition-all" onClick={(e) => { e.stopPropagation(); navigate(1); }}><ChevronRight size={56} strokeWidth={1} /></button>
              </>
            )}
